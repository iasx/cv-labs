from numpy import all, any, array, arctan2, cos, sin, exp, dot, log, logical_and, roll, sqrt, stack, trace, unravel_index, pi, deg2rad, rad2deg, where, zeros, floor, full, nan, isnan, round, float32, mean, cov
from numpy.linalg import det, lstsq, norm
from cv2 import resize, GaussianBlur, subtract, KeyPoint, INTER_LINEAR, INTER_NEAREST, PCACompute
...
def computeKeypointsAndDescriptors(image, sigma=1.6, num_intervals=3, assumed_blur=0.5, image_border_width=5):
    ...
    gaussian_images = generateGaussianImages(
        base_image,
        num_octaves,
        gaussian_kernels
    )
...
def generateGaussianImages(image, num_octaves, gaussian_kernels):
    ...
    for octave_index in range(num_octaves):
        ...
        image = resize(octave_base, (int(
            octave_base.shape[1] / 2), int(octave_base.shape[0] / 2)), interpolation=INTER_NEAREST)
...
def computeKeypointsWithOrientations(keypoint, octave_index, gaussian_image, radius_factor=3, num_bins=36, peak_ratio=0.8, scale_factor=1.5):
    ...
    orientation_peaks = where(logical_and(smooth_histogram > roll(
